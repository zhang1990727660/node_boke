...
exports.insertComment = ( value ) => {
  let _sql = "insert into comment set userId=?,userName=?,content=?,moment=?,postid=?,avator=?;"
  return query( _sql, value )
}
// 通过名字查找用户
...
// 查询所有文章
exports.findAllPost = () => {
  let _sql = `select * from posts order by moment desc;`
  return query( _sql)
}
// 查询所有文章数量
...
// 查询个人分页文章
exports.findPostByUserPage = (uid,page) => {
  let _sql = ` select * from posts where uid="${uid}" order by id desc limit ${(page-1)*10},10 ;`
  return query( _sql)
}
// 更新修改文章
